                        ...
                        span.pages.controls
                            template(v-for="page in range(1, pager.pageCount)")
                                a.page.first.before-selected(v-if="pager.currentPage !== 1 && page === 1" href="#" title="Go to page 1" @click="toPage(page)")
                                    span.cl.pl {{ page }}
                                a.page.last.after-selected(v-if="pager.currentPage !== pageCount && page === pageCount" href="#" :title="goToPageString(page)" @click="toPage(page)")
                                    span.cl.pl {{ page }}
...
export default {
    ...
    methods: {
        ...
        async search () {
            ...
            this.pager.pageCount = (this.companies.length / this.pager.perPage)
